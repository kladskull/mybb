...
extern mybbfloat asc2flt(char *str);
extern int chaninfo;
...
void buffermeta(statement *stmt, int type, char *c)
{
	...
	{
		if ((chaninfo == 1) || (!strcmp(c, "ERR")))
			x=0; 
		else 
			x=15;

		for (x=x; *optable[x].symbol; x++)
			if (!strcmp(c, optable[x].symbol))
				op = optable[x].opcode; 
	}
	...
	if ((op == OP_BNK) || (op == OP_DOM) || (op == OP_END) ||
		...
		(op == OP_LEN) || (op == OP_OPT) || (op == OP_ATR) || (op == OP_SRT) || 
		(op == OP_SEP)) 
	{
		stmt->metapos--;
	...
	{
		if ((lastopcode != CMD_LET) && (lastopcode != CMD_DEFFN) &&	(lastopcode != CMD_FOR))
			op = OP_EQUALSCMP;
		else 
			return; 
	} else if (op == OP_ADD) 
	...
	if (c[0] == '@') 
	{
		if (numargs[parencount] == 2) 
			...
			stmt->metalist[stmt->metapos].operation = 0xF6;
	...
	if (!op)
	{
		...
		case TOK_SETVAL:
			...
			if (c[strlen(c)-1] == '$') 
			{
				...
				{ 
					stmt->metalist[stmt->metapos].operation = SETVAL_NUMERICARRAY;
		...
		case TOK_COMMA:
			if (lastopcode == CMD_PRINT || lastopcode == CMD_INPUT || lastopcode == CMD_IF)
				...
				stmt->metalist[stmt->metapos].operation = 0xEC;
		...
		case TOK_VARIABLE:
			...
			{     
				...
				if (numargs[parencount] >= 30) 
				{
					stmt->metalist[stmt->metapos].operation = GETVAL_NUMERICARRAY;
					stmt->metalist[stmt->metapos].shortarg += 0x8000;
					numargs[parencount] -= 30;
				} else if (numargs[parencount] >= 20)
				{
...
int addfunction(program *pgm, char *name, unsigned short lineref, int defined)
{
	...
	if (defined) 
		pgm->userfunctions[pgm->numfunctions].defined = 1;
	...
	do 
	{
		if (stmttemp->linenum == line)
		{
			...
			{        
				...
				if ((stmttemp->opcode == CMD_DEFFN) && (stmttemp->metalist[x].operation == 0x8DC9))
			...
			if (stmttemp == pgm->firststmt)
				...
				stmtlast->nextstmt = stmttemp->nextstmt;
...
skip:
		if (curstmt->linenum >= startline) 
		{
			...
			envinfo.gotoline = 0;
			if (!execline(pgm, curstmt)) 
				goto done;
			if (envinfo.allowexec == 0)
			{ 
				envinfo.gotoline = pgm->lastexec; envinfo.allowexec = 1; 
				return; 
			}
...
restart2:
	...
	for (x=1; x<pgm->numfunctions+1; x++)
	{ 
		if (pgm->userfunctions[x].numrefs == 0)
		{ 
			delfunction(pgm, x); 
			goto restart2; 
		} 
...
restart3:
	for (x=1; x<pgm->numlabels+1; x++)
	{ 
		if (pgm->labels[x].numrefs == 0) 
		{ 
			dellabel(pgm, x); goto restart3;
		} 
	}
	...
	do 
	{
		...
		for (y=1; y<tempstmt->metapos; y++)
		{
			if ((tempstmt->metalist[y].operation > 0x8D00 &&
				...
				WriteByte(fp, tempstmt->metalist[y].operation);
			...
			{
				if (tempstmt->metalist[y].operation == MNEMONICREF && (strlen(tempstmt->metalist[y].stringarg) <= 2))
					fwrite(&tempstmt->metalist[y].stringarg, 2, 1, fp);
				else if(tempstmt->opcode == CMD_REM || tempstmt->opcode == CMD_REM2)
					fwrite(&tempstmt->metalist[y].stringarg, tempstmt->metalist[y].shortarg, 1, fp);
				else 
...
{ 
	...
	do 
	{
		...
		if (tempstmt->length == 0xF3)
		{
			...
			{ 
				fseek(fp, -1, SEEK_CUR); 
				tempstmt->length = 0xF3;
			}
		}

		tempstmt->linenum = ReadShort(fp);
		
		if (tempstmt->linenum == 0xFFFF)
			laststmt = 1;
...
getopcode:
		...
		for (y=1; y<MAX_STMT_METAS; y++)
		{
			...
			tempstmt->metapos++;
			if (tempstmt->metalist[y].operation == 0xF2)
				break;
			...
			{
				if (fnctable[get_fnc(get_fncname(tempstmt->metalist[y].operation))].numparms == 99) 
					tempstmt->metalist[y].intarg = ReadByte(fp);
			} else 
				switch(tempstmt->metalist[y].operation)
				{
					case 0x8DC9:
...
getnext:
		if (!pgm->firststmt) 
			pgm->firststmt = tempstmt;
		else 
			templast->nextstmt = tempstmt;
	...
	if (labeltab) 
	{
		...
		do 
		{
			...
			fread(&pgm->labels[pgm->numlabels].name, pgm->labels[pgm->numlabels].length, 1, fp); 
			...
			if (pgm->labels[pgm->numlabels].lineref != 0) 
				...
				pgm->labels[pgm->numlabels].defined = 0;
	...
	if (pgm->numfunctions) 
	{
		...
		for (x=1; x<pgm->numfunctions+1; x++)
		{
			...
			if (pgm->userfunctions[x].lineref != 0) 
				pgm->userfunctions[x].defined = 1;
		}
		ReadByte(fp);
	}
