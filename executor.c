...
mybbstring fnc_strcat(mybbstring a, mybbstring b);
...
mybbfloat arith(mybbfloat val1, mybbfloat val2, int op);
int compare(mybbfloat val1, mybbfloat val2, int op);
void gosubpush(int value);
int gosubpop(void);
extern int forlist(statement *stmt, int start);
int strcompare(mybbstring val1, mybbstring val2, int op);
...
void strexecpush3(mybbstring str)
{
...
void fltexecpush(mybbfloat value)
{
	reduce(&value);
...
mybbfloat add(mybbfloat val1, mybbfloat val2)
{
	mybbfloat result;
...
mybbfloat multiply(mybbfloat val1, mybbfloat val2)
{
	mybbfloat result;
...
mybbfloat divide(mybbfloat val1, mybbfloat val2)
{
	mybbfloat result;
	...
	if (val1.mantisa.i != 0) 
	{
		...
		} while (val1.mantisa.i < 214748364); // make sure value doesn't overflow
	}                                       

	if (val2.mantisa.i == 0) 
		rterror(ERR_NUM_VALUE_OVERFLOW);
...
mybbfloat subtract(mybbfloat val1, mybbfloat val2)
{
	mybbfloat result;
...
mybbfloat exponent(mybbfloat val1, mybbfloat val2)
{
	mybbfloat result;
	double x;
...
mybbfloat arith(mybbfloat val1, mybbfloat val2, int op)
{
	...
	switch(op) 
	{
		...
		case OP_XOR:
			if ((val1.mantisa.i || val2.mantisa.i) && !(val1.mantisa.i && val2.mantisa.i)) 
				endval = int2flt(1);
			else 
				endval = int2flt(0);
			break;
...
void exec_userfunction(program *pgm, statement *stmt, int start, int end)
{
	...
	b = execcounter;
	for (a=start+1; a<end+1; a++) 
		a = a + simplify(pgm, stmt, a); 
	...
	for (y=0; y<=tmpstmt.metapos; y++)
		if (tmpstmt.metalist[y].operation == 0xF5) 
			break;      
	
	for (a=y; a<=tmpstmt.metapos; a++)
		if (tmpstmt.metalist[a].operation == 0xF8) 
			break;
...
int simplify(program *pgm, statement *stmt, int x)
	...
	if (get_fncname(stmt->metalist[x].operation) || get_sysvarname(stmt->metalist[x].operation)) 
	{
		if ((stmt->metalist[x].operation == OP_AND) || (stmt->metalist[x].operation == OP_OR) || (stmt->metalist[x].operation == OP_XOR)) 
		{
			goto blah;
		}
...
blah:
	switch(stmt->metalist[x].operation)
	{
		...
		case 0x8DC9:
			...
			{
				if (stmt->metalist[y].operation == 0xF5) 
					a++;

				if (stmt->metalist[y].operation == 0xF8) 
					a--;
		...
		case GETVAL_STRING: 
			...
			if (stmt->metalist[x].shortarg & 0x8000) 
			{
				...
				if (stmt->metalist[x].operation == GETVAL_STRINGARRAY) 
				{
					flt = fltexecpop();
					flt2 = fltexecpop();
					tmpsym = get_arraysym(idx2sym(pgm, index), flt2int(flt), flt2int(flt2),	flt2int(fltexecpop()));
				} else {
					tmpsym = get_arraysym(idx2sym(pgm, index), -1, -1, flt2int(fltexecpop()));
				}
			} else if (stmt->metalist[x].operation == GETVAL_STRINGARRAY) 
			{
				flt = fltexecpop();
				tmpsym = get_arraysym(idx2sym(pgm, index), -1, flt2int(flt), flt2int(fltexecpop()));
			} else {
			...
			if ((start > tmpsym->string.len) || (end > tmpsym->string.len))
				rterror(ERR_INVALID_SUBSTRING_REF);
...
int strcompare(mybbstring val1, mybbstring val2, int op)
{
	...
	{
		case OP_EQUALSCMP:
			if (!strcmp(val1.str, val2.str)) 
				return 1;
			break;

		case OP_GREATTHAN:
			if (strcmp(val1.str, val2.str) > 0) 
				return 1;
			break;
...
void dprintf(FILE *fp, char *fmt, ...)
{
	...
	if (fp == stdout) 
	{
		...
		{
			printf("<ENTER> or CTRL-L For Non-Stop");
			if (readkey() == 0xC) 
				linectr = 100;
			else 
				linectr = 0;
...
int exec_dump(program *pgm, statement *stmt)
{
	...
	dprintf(fp, "! ERR=%d\n", envinfo.lasterror);
	//dprintf(fp, "! Last Executed Line = %d   \n", pgm->lastexec);
	//dprintf(fp, "! Gotoline = %d   \n", envinfo.gotoline);
	...
	if (pgm->numfunctions > 0) 
	{
		for (x=1; x<=pgm->numfunctions; x++) 
		{
			if (pgm->userfunctions[x].defined == 1) 
				dprintf(fp, "! Function: %s (line %d)\n", pgm->userfunctions[x].name, pgm->userfunctions[x].lineref);
		}
	}
	if (gosubcounter) 
	{
		for (x=0; x<gosubcounter; x++) 
		dprintf(fp, "! GOSUB on line %d going to line xxx\n", gosubstack[x]);
	}
	
	if (whilecounter) 
	{
		for (x=0; x<whilecounter; x++) 
			dprintf(fp, "! WHILE/WEND loop starting at line %d\n", whilestack[x]);
	}
	...
	if (fcounter) 
	{
		...
		{
			dprintf(fp, "! FOR/NEXT loop starting at line %d\n", fstack[x].repeatline-1);
			dprintf(fp, "! Variable   = %s    \t", fstack[x].storeto->name);
			dprintf(fp, "! Step value = %s    \t", flt2asc(fstack[x].stepval));
	...
	{
		dprintf(fp, "! Open Channels\n");
		dprintf(fp, "! \n");
		dprintf(fp, "! Files\n");
		dprintf(fp, "!  Channel\tFile Name\n");
		do 
		{ 
			if (tmpfile->channel == 0x7FBC) 
				break;
			else 
				dprintf(fp, "!     %.5d\t%s\n", tmpfile->channel,tmpfile->filename);
		...
		if (top->string.len != 0) 
		{
			...
			for (a = 0; a < top->string.len; a++) 
			{
				...
				if (isprint(top->string.str[a])) 
				{
					...
					y += sprintf(hexstring+y, "%c%c", tmp[strlen(tmp)-2], tmp[strlen(tmp)-1]); 
					z++;
				}
...
void dump_var(FILE *fp, symbol *top)
{
	...
	if (top->arraylink) 
	{
		dprintf(fp, "DIM %s[", top->name);
		dprintf(fp, "0:%d", top->dim1-1);
	
		if (top->dim2 != -1) 
			dprintf(fp, ",0:%d", top->dim2-1);

		if (top->dim3 != -1) 
			dprintf(fp, ",0:%d", top->dim3-1);
			...
			if (top->dim2 != -1) 
			{
				...
				for (b = 0; b < top->dim2; b++) 
				{
					if (top->dim3 != -1) 
					{
						...
						{
							dprintf(fp, "%s[%d,%d,%d]",top->name, a, b, c);
							dump_sym(fp, tmpsym3, 0, type);
							tmpsym3 = tmpsym3->nextsym;
						}
					} else { 
						dprintf(fp, "%s[%d,%d]",top->name, a, b);
