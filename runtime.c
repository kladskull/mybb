...
mybbfloat add(mybbfloat flt1, mybbfloat flt2);
...
void rterror(int code)
{
	...
	} else 
	{
		for (x=0; x<65; x++)
		{
			...
			{  
				printf("\nError #%d: %s\n", errortable[x].errorcode, errortable[x].errortext);
...
int execline(program *pgm, statement *stmt)
{
	...
	if (stmt->opcode != CMD_IF && stmt->opcode != CMD_ENTER &&  stmt->opcode != CMD_DEFFN) 
	{    
		if (stmt->linenum) 
		{
	...
	switch(stmt->opcode) 
	{
		...
		case CMD_RETRY:
			...
			if (!envinfo.lasterrorline) 
				rterror(ERR_RETURN_WITHOUT_GOSUB);
...
void exec_dumpstmt(program *pgm, statement *stmt)
{
	...
	do 
	{
		if (y == tempstmt->linenum)
			dumpstmt(tempstmt, fp);
	...
	{ 
		if ((stmt->metalist[x].operation == 0xEC) || (x == stmt->metapos))
		{
			...
			if (elmt.type == 2)
			{
				...
				if (start == 0 && end == tmpsym->string.len) 
					tmp = elmt.strarg;
				else 
				{
					...
					for (y = 0; y < tmpsym->string.len; y++) 
					{
						if (y >= start && y < end) 
						{
							if (elmt.strarg.len == 0)
								tmp.str[y] = ' ';

							else tmp.str[y] = elmt.strarg.str[count];
							count++;
						} else 
							tmp.str[y] = tmpsym->string.str[y];
					}
				}

				tmpsym->string = tmp;
			} else 
				tmpsym->numeric = elmt.fltarg;

			if (x == stmt->metapos)
				break;
		}

		if (stmt->metalist[x].operation == SETVAL_STRING || stmt->metalist[x].operation == SETVAL_STRINGARRAY || stmt->metalist[x].operation == SETVAL_NUMERIC || stmt->metalist[x].operation == SETVAL_NUMERICARRAY) 
		{
			...
			{ 
				storeto -= 0x8000;
				if (stmt->metalist[x].operation == SETVAL_STRINGARRAY || stmt->metalist[x].operation == SETVAL_NUMERICARRAY) 
				{
				...
				{
					tmpsym = get_arraysym(idx2sym(pgm, storeto), -1, -1, flt2int(fltexecpop()));
				}
			} else if (stmt->metalist[x].operation == SETVAL_STRINGARRAY || stmt->metalist[x].operation == SETVAL_NUMERICARRAY) 
			{
				tmpsym = get_arraysym(idx2sym(pgm, storeto), -1, flt2int(fltexecpop()), flt2int(fltexecpop()));
			} else {
...
int exec_print(statement *stmt)
{
	...
	if (stmt->metalist[stmt->metapos-1].operation != 0xF1) 
		fprintf(fp, "\n");
...
int exec_save(program *pgm, statement *stmt)
{
	...
	{
		s = strexecpop();
		if (!writeall(pgm, s.str))
			return 0;

	} else if (pgm->filename) 
	{
		if (!writeall(pgm, pgm->filename))
			return 0;
...
void exec_delete(program *pgm, statement *stmt)
{
	...
	if (execcounter)
	{
		if (execcounter == 2)
		{
			...
			deletestmt(pgm, flt2int(fltexecpop()));
...
int exec_if(program *pgm, statement *stmt)
	...
	for (x=0; x<stmt->metapos; x++) 
		if (stmt->metalist[x].operation == 0xE7) 
			break;
	...
	for (x=y; x<stmt->metapos; x++) 
	{
		...
		if (stmt->metalist[x].operation == 0xE2)
			ctr -= 2;
	...
	{  
		...
		{
			if (stmt->metalist[x].operation == MNEMONICREF && !stmt->metalist[x].intarg)
			{ 
...
int exec_for(program *pgm, statement *stmt)
{
	...
	if (flt2dbl(tmp.startval) > flt2dbl(tmp.endval))
		tmp.negative = 1;
	else 
		tmp.negative = 0;
	...
	if (stmt->metalist[1].operation == SETVAL_NUMERIC) 
	{
		if (stmt->metalist[1].shortarg == tmp.storeto->idx)
		{
			...
			if (tmp.negative)
			{
				...
				{ 
					envinfo.gotoline = stmt->linenum+1; 
					return 1; 
				}
				else 
				{ 
					forpush(tmp); 
					envinfo.gotoline = tmp.repeatline;
					return 1; 
				}
			} else 
			{
...
int exec_input(program *pgm, statement *stmt)
{
	...
	for (x=tmp; x<stmt->metapos; x++)
	{
		...
		if (stmt->metalist[x].operation == SETVAL_NUMERIC || stmt->metalist[x].operation == SETVAL_NUMERICARRAY || stmt->metalist[x].operation == SETVAL_STRINGARRAY || stmt->metalist[x].operation == SETVAL_STRING) 
		{
			storeto = stmt->metalist[x].shortarg;
			if (stmt->metalist[x].shortarg & 0x2000) 
				storeto -= 0x2000;
...
again:
				...
				if (stmt->metalist[x].operation == SETVAL_STRING || stmt->metalist[x].operation == SETVAL_STRINGARRAY) 
		...
		} else if (stmt->metalist[x].operation == 0xF1 || stmt->metalist[x].operation == 0xF2) 
		{
			elmt = execpop();
			if (elmt.type == 2) 
			{
...
int exec_open(statement *stmt)
{
	...
	if (get_chaninfo(envinfo.curchannel) != NULL) 
		rterror(ERR_FILE_DEVICE_USAGE);
	...
	for (x=0; x<8; x++) 
		newfile->filename[x] = '\0';

	for (x=0; x<tmp.len; x++) 
		newfile->filename[x] = tmp.str[x];

	newfile->nextfile = 0;

	if (envinfo.curopt) 
	{
		if (!strcmp(envinfo.curopt, "TEXT"))
			goto textfile;
	}
	...
	if (!strcmp(tmp.str, newfile->filename)) 
	{
		...
		{ 
			...
			newfile->recordsize = ReadLong(newfile->fp);
...
textfile:
		if (envinfo.curisz > 0) 
			newfile->indexsize = envinfo.curisz;
		else 
			newfile->indexsize = 0;
	...
	do 
	{
		if (tempfile->channel == envinfo.curchannel)
		{
			if (envinfo.curchannel == envinfo.firstfile->channel)
				...
				templast->nextfile = tempfile->nextfile;
...
int exec_text(statement *stmt)
{
	...
	if ((fp = fopen(tmp.str, "r+")) != NULL) 
		rterror(ERR_BAD_FILEID);

	if ((fp = fopen(tmp.str, "w")) == NULL) 
		rterror(ERR_OUT_OF_DISK_SPACE);
...
void exec_read(program *pgm, statement *stmt)
{
	...
	else if (tmpfile->type == FILE_INDEXED)
	{
		if (envinfo.curindex == -1)
			recordnum = tmpfile->nextrecord;
		else 
			recordnum = envinfo.curindex;
...
raw:
	for (x=0; x<stmt->metapos; x++) 
	{
		if ((stmt->metalist[x].operation == GETVAL_STRING) || (stmt->metalist[x].operation == GETVAL_NUMERIC)) 
		{
			...
			do
			{
				...
				if ((ch == sep) || (ch == 0x0A) || (ch == 0x0D)) 
					break; 

				out[ctr] = ch;
				ctr++;

				if ((recordlen > 0) && (ctr > recordlen))
					rterror(ERR_END_OF_RECORD);
...
int exec_readrecord(program *pgm, statement *stmt, byte update)
{
	...
	else if (tmpfile->type == FILE_INDEXED) 
	{
		...
		realloc(tmpsym->string.str, tmpsym->string.len);
...
void exec_write(statement *stmt)
{
	...
	else if (tmpfile->type == FILE_INDEXED)
	{
		...
		if (recordnum+1 > tmpfile->numrecords)
			rterror(ERR_END_OF_FILE);
		...
		for (x = 0; x < y; x++)
		{
			elmt = execpop();
			if (elmt.type == 1)
				s.str = flt2asc(elmt.fltarg);
			else
				s = elmt.strarg;

			if (offset+s.len> tmpfile->recordsize)
				rterror(ERR_END_OF_RECORD);
...
void exec_enter(program *pgm, statement *stmt)
{
	...
	if (pgm == gprog) 
		rterror(ERR_CALL_ENTER_MISMATCH);
...
void exec_exit(program *pgm, statement *stmt)
{
	...
	} else 
	{
		do 
		{
			if (tmpstmt2->linenum == pgm->prevprog->lastexec) 
				break;
		...
		{
			if (tmpstmt2->metalist[x].operation == 0xEC || tmpstmt2->metalist[x].operation == 0xF2) 
			{
				if (tmpstmt2->metalist[x-1].operation == GETVAL_NUMERIC) 
				{
					tempsym = idx2sym(pgm->prevprog, tmpstmt2->metalist[x-1].shortarg);
					tempsym->numeric = fltexecpop();
...
void exec_on(program *pgm, statement *stmt)
{
	...
	for (y = 0; y <= stmt->metapos; y++) 
		if ((stmt->metalist[y].operation == 0x00F4)  || (stmt->metalist[y].operation == 0x01F4)) 
			break;
