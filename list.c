...
void listline(char *var, statement *stmt, byte showline)
{
	...
	if (stmt->errorflag) 
	{
		mysprintf(var, "Error #20: Syntax Error ... ");
	}

	if (showline) 
		mysprintf(var, "%.5d ", stmt->linenum);
	...
	switch(stmt->opcode) 
	{
		...
		case CMD_GOSUB:
			if (stmt->metalist[1].operation == LABELREF)
				...
				mysprintf(var, "%.5d", stmt->metalist[1].shortarg);
			break;
		...
		case CMD_DELETE:          
			if (stmt->metapos == 1)  
				...
				mysprintf(var, "%.5d, %.5d", flt2int(stmt->metalist[1].floatarg), flt2int(stmt->metalist[3].floatarg));
			break;
...
void list_on(char *var, statement *stmt)
{
	...
	for (x=0; x<=stmt->metapos; x++)
		if ((stmt->metalist[x].operation == 0x00F4) || (stmt->metalist[x].operation == 0x01F4)) 
			break;
...
void list_deffn(char *var, statement *stmt)
{
	...
	for (x=0; x<=stmt->metapos; x++)   
		if (stmt->metalist[x].operation == 0xF5) 
			break; 
...
void simplifylist(char *var, statement *stmt, int start, int end)
{
	...
	{            
		...
		{
		if ((stmt->metalist[y].operation == OP_AND) || (stmt->metalist[y].operation == OP_OR) || (stmt->metalist[y].operation == OP_XOR)) 
			goto blah;
		...
		if (stmt->metalist[y].intarg) 
		{
			...
			for (ctr=0; ctr<fnctable[get_fnc(get_fncname(stmt->metalist[y].operation))].numparms; ctr++) 
			{ 
				reorder[ctr] = SafeMalloc(MAX_STRING_LENGTH); 
				reorder[ctr] = listpop(); 
			}
...
blah:
		switch(stmt->metalist[y].operation) 
		{
			...
			case 0xF8: 
				if (stmt->opcode != CMD_DEFFN) 
				{
					...
					do 
					{
						reorder[ctr] = SafeMalloc(MAX_STRING_LENGTH);
						reorder[ctr] = listpop(); 
						if (!strcmp(reorder[ctr], "<BEGINLIST>"))
							break;
			...
			case SETVAL_STRING:
				...
				{
					ctr -= 0x8000;
					if (stmt->metalist[y].operation == GETVAL_STRINGARRAY || stmt->metalist[y].operation == SETVAL_STRINGARRAY) 
					{
						tmp1 = listpop(); 
						tmp2 = listpop();
				...
				} else if (stmt->metalist[y].operation == GETVAL_STRINGARRAY || stmt->metalist[y].operation == SETVAL_STRINGARRAY) 
				{
					tmp1 = listpop();
				...
				if ((stmt->metalist[y].operation == SETVAL_STRING) || (stmt->metalist[y].operation == SETVAL_STRINGARRAY)) 
				{
			...
			case SETVAL_NUMERICARRAY:
				...
				{
					ctr -= 0x8000;
					if (stmt->metalist[y].operation == GETVAL_NUMERICARRAY || stmt->metalist[y].operation == SETVAL_NUMERICARRAY) 
					{
						tmp1 = listpop(); 
						tmp2 = listpop();
						sprintf(tmp, "[%s,%s,%s]", listpop(), tmp2, tmp1);
				...
				} else if (stmt->metalist[y].operation == GETVAL_NUMERICARRAY || stmt->metalist[y].operation == SETVAL_NUMERICARRAY) 
					{
					tmp1 = listpop();
					sprintf(tmp, "[%s,%s]", listpop(), tmp1);
					listpush(tmp);
				...
				if (stmt->metalist[y].operation == SETVAL_NUMERIC || stmt->metalist[y].operation == SETVAL_NUMERICARRAY) 
					if ((stmt->opcode == CMD_LET) || (stmt->opcode == CMD_FOR))
						strcat(tmp, "=");
